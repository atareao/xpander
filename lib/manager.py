...
class Config(object):
    ...
    def __init__(self):
        self.config_path = os.path.join(shared.config['config_dir'],
                                        'xpander.json')
        try:
            self.read_config()
    ...
    def write_config(self):
        with open(self.config_path, 'w') as config:
            config.write(json.dumps(shared.config,
...
class Phrases(object):
                ...
                if ext == '.json':
                    with open(entry.path) as phrase_file:
                        ...
                        shared.phrase_paths[phrase['name']] = os.path.relpath(
                            ...
                            shared.config['phrases_dir'])

    def new_phrase(self, name, string, body, command, method, window_class,
                   window_title, path):
        phrase = {}
        phrase['name'] = name
        ...
        file_path = os.path.join(shared.config['phrases_dir'], path,
                                 name + '.json')
    ...
    def edit_phrase(self, name, **kwargs):
        phrase = next((phrase for phrase in shared.phrases
                      if phrase['name'] == name))
        for arg in kwargs:

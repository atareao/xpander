...
class Service(threading.Thread):
    ...
    def __init__(self):
        ...
        self.method = (self.type_string,
                       self.send_clipboard,
                       self.send_clipboard_terminal)
    ...
    def handle_event(self, keypress, keysym, raw_key, modifiers):
        if keypress:
            ...
            if shared.service_running:
                ...
                if char and not (modifiers['<Alt>'] or
                    ...
                    if char not in {' ', '\n', '\t'}:
                        ...
                        if phrase:
                            if ((not phrase['window_class'] and
                                 not phrase['window_title']) or
                                ...
                                if '$|' in phrase['body']:
                                    if phrase['command']:
                                        ...
                                        output = self.get_output(
                                            phrase['body'])
                                        string, count = self.get_caret_pos(
                                            output)
                                        self.store_clipboard()
    ...
    def toggle_service_hotkey(self):
        ...
        GLib.idle_add(shared.menu_toggle_service.set_active,
                      shared.service_running)

    def grab_hotkeys(self):

        pause_keysym = shared.interface.char_to_keysym(
            shared.config['key_pause'][0])
        if pause_keysym:
        ...
        if manager_keysym:
            manager_keycode = shared.interface.keysym_to_keycode(
                manager_keysym)[0]
            manager_state = 0
            for mod in shared.config['key_show_manager'][1]:
